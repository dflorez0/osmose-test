function biomoo_postcompute(project)
	-- operating costing
	local opcost = project.results.opcost[1]
	...
	for i, unit in ipairs(project.units[1]) do 
	  table.insert(cinv, {unit_name = unit.name, value = (unit.cost[1]*(1/(unit.mult_t[1]+0.00001)))*(unit.mult_t[1]^alpha)})
	end
	local tot_ic = 0
	...
	return {total_op_cost, 1/total_op_cost}
end